...
use Modules\Brand\Application\Schema\Item\ListQueryHandlerSchema;
...
final readonly class ListQueryHandler extends AbstractListQueryHandler
{
     ...
     * Get the list  entity
     *
     * @param BrandListQuery $query
     *
     * @return ListQueryResponseWrapper<BrandDTO>
     * @throws Exception
     */
    #[QueryHandler]
    public function __invoke(BrandListQuery $query): ListQueryResponseWrapper
    {
        $scope = [];
        if (!$query->withTrash) {
            $scope = [
    ...
    protected function mapToArrayInput(BrandListQuery $query): array
    {
        return [
